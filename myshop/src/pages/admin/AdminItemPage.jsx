...
const AdminItemPage = () =>{
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getItem(id, token)
      .then(data => setFormData({ ...formData, ...data, image: null, imagePath: data.imagePath || "" }))
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      {formData.imagePath && <img src={formData.imagePath} alt="Preview" className="w-full h-40 object-cover mb-2 rounded" />}
      <form onSubmit={handleSubmit} className="space-y-3">
        {["name", "brand", "type", "description", "warranty"].map(f => (
          <input key={f} type="text" name={f} placeholder={f.charAt(0).toUpperCase() + f.slice(1)}
            value={formData[f]} onChange={handleChange} className="border p-2 w-full rounded" />
        ))}
